...
def check_raw(raw:str)->Optional[V2Config]:
    ...
    if CUSTOM_SNI:
        ...
        if len(compat)<mode["min_compat"]:
            # إذا probe فشل كلياً — قد يكون GitHub يحجب
            # نقبل إذا TCP ناجح وهو CF IP معروف
            if is_cf and ping < 300:
                log.debug(f"  ⚠️ probe fail but CF+fast_tcp — accepting {host}")
                compat = ["(cf-detected)"]
                best   = "(cf-detected)"
                probe_ms = ping
            else:
