...
# ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
_PROV_PATS = (
    "speedtest.","ookla.","fast.com","akamai","amazonaws",
    "azureedge","gstatic","googlevideo","fbcdn","fbsbx",
    "twimg","t.co","whatsapp.net","zain.com","oodi.com.sa",
    "mobily.com.sa","ooredoo.qa","gov.","msecnd.",
)

def is_embedded(raw: str) -> bool:
    sni = extract_sni(raw).lower()
    if not sni: return False
    if any(sni == bh.lower() for bh in ALL_BUG_HOSTS): return False
    return any(p in sni for p in _PROV_PATS)
...
def ws_probe(host: str, port: int, bug_host: str) -> Optional[int]:
    """
    ══════════════════════════════════════════════════════════
    الفحص الحقيقي — يضمن Downlink ≠ 0B قبل النشر.

    منطق القبول الصارم:
    ✅ 101 Switching Protocols = WebSocket ناجح تماماً (أفضل حالة)
    ✅ 200 + CF headers = CF worker يستجيب
    ✅ 400 + CF-Ray header = CF يرد لكن path خاطئ (يُصلح بـ /linkvws)
    ❌ 403/404 بدون CF-Ray = سيرفر آخر يحجب = Downlink=0B
    ❌ 530 = CF يرفض SNI = Downlink=0B
    ❌ 5xx/فارغ/timeout = ميت

    المبدأ: CF-Ray header هو الدليل الوحيد أن الـ proxy يعمل.
    ══════════════════════════════════════════════════════════
    """
    if not deadline_ok(): return None
    ctx = _ssl_ctx()
    # جرّب مسارات السيرفر الشغال أولاً
    for path in ("/linkvws", "/ws", "/vws", "/"):
        try:
            t0   = time.perf_counter()
            ...
            while time.perf_counter() < dl:
                try:
                    ...
                    if not chunk: break
                    resp += chunk
                    if b"\r\n\r\n" in resp: break  # نهاية الـ headers
            ...
            # ── الدليل الأقوى: CF-Ray header موجود ──────────────────────
            has_cf_ray    = b"cf-ray:" in resp_lower
            has_cf_server = b"server: cloudflare" in resp_lower
            has_cf_cache  = b"cf-cache-status" in resp_lower
            is_real_cf    = has_cf_ray or has_cf_server or has_cf_cache

            # ── قرار القبول ─────────────────────────────────────────────
            if status == 101:
                # WebSocket Upgrade ناجح = أفضل حالة ممكنة
                log.debug(f"  ✅101 WS {host}←{bug_host} {path} ({elapsed}ms)")
                ai_bug_update(bug_host, True)
                return elapsed

            if status == 200 and is_real_cf:
                log.debug(f"  ✅200+CF {host}←{bug_host} ({elapsed}ms)")
                ai_bug_update(bug_host, True)
                return elapsed

            if status == 400 and is_real_cf:
                # CF يرد 400 Bad Request = SNI صحيح لكن path/upgrade غير مدعوم
                # يعمل مع تطبيقات v2ray لأنها تتعامل مع الـ WebSocket مباشرة
                log.debug(f"  ✅400+CF {host}←{bug_host} ({elapsed}ms)")
                ai_bug_update(bug_host, True)
                return elapsed

            if status == 426 and is_real_cf:
                # Upgrade Required = CF يطلب WebSocket upgrade → يعمل
                ai_bug_update(bug_host, True)
                return elapsed

            # ── رفض صريح ────────────────────────────────────────────────
            # 530 = CF Error Page = SNI خاطئ = Downlink=0B بالتأكيد
            # 403/404 بدون CF headers = reverse proxy آخر يحجب
            # 5xx = سيرفر ميت
            log.debug(f"  ❌{status} {host}←{bug_host} cf={is_real_cf}")

        except Exception as e:
            log.debug(f"  ❌err {host}←{bug_host}: {type(e).__name__}")

    ai_bug_update(bug_host, False)
    return None

def multi_probe(host: str, port: int) -> tuple[list[str], str, int]:
    """يفحص كل Bug Host بالتوازي — سريع ودقيق."""
    if not deadline_ok(): return [], "", 0
    ordered  = ai_order()
    timings: dict[str, int] = {}
    with ThreadPoolExecutor(max_workers=min(len(ordered), 12)) as ex:
        futs = {ex.submit(ws_probe, host, port, bh): bh for bh in ordered}
...
def check_raw(raw: str) -> Optional[V2Config]:
    ...
    if CUSTOM_SNI:
        ...
        # يجب أن ينجح على الأقل هوستين = دليل أن السيرفر حقيقي
        if len(compat) < 1: return None
        # إذا ping عالية + compat قليلة = احتمال Downlink=0B → رفض
        if ping > 300 and len(compat) < 2: return None

    # ── 4b. Strict path check — تحقق أن path الكونفيج يعمل ─────────────────
    # إذا best_probe_ms عالي جداً = اتصال بطيء = قد لا يعمل
    if probe_ms > 1800: return None   # > 1.8s = slow = reject
...
def main() -> None:
    ...
    # ── 1. Load cache ─────────────────────────────────────────────────────
    cached = cache_to_configs(cache_load())
    if cached: log.info(f"♻️  {len(cached)} cached configs loaded")
