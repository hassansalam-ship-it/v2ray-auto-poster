...
def ws_probe(host:str, port:int, bug_host:str, path:str="/ws") -> Optional[int]:
    """
    DUAL PROBE: WS Upgrade + HTTPS fallback
    
    المشكلة: GitHub Actions يحجب WS على CF IPs أحياناً
    الحل: نجرب أولاً WS، إذا فشل نجرب HTTPS GET
    CF يرد على HTTPS GET بـ CF-Ray header = كافٍ للتأكيد
    """
    ...
    # ── Phase 1: WS Upgrade (أفضل) ────────────────────────────────
    for try_path in ai_paths[:4]:
        try:
            t0   = time.perf_counter()
            ...
            if status == 200 and has_ws:
                ai_bug_update(bug_host, True); ai_path_win(try_path)
                return elapsed
            if status in (400, 200, 426) and is_cf:
                ai_bug_update(bug_host, True); ai_path_win(try_path)
                return elapsed

        except Exception: pass

    # ── Phase 2: HTTPS GET fallback (يكتشف CF حتى لو WS محجوب) ───
    try:
        t0   = time.perf_counter()
        conn = socket.create_connection((host, port), timeout=PROBE_TIMEOUT)
        conn.settimeout(PROBE_TIMEOUT)
        ...
        if resp:
            ...
            parts  = first.split()
            status = int(parts[1]) if len(parts)>=2 and parts[1].isdigit() else 0
            # CF يرد بأي status مع CF-Ray = CF proxy شغال
            if is_cf and status not in (0, 530):
                ai_bug_update(bug_host, True)
                log.debug(f"  ✅HTTPS+CF {host}←{bug_host} s={status} {elapsed}ms")
                return elapsed
    except Exception: pass
