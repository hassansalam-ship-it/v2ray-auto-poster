...
def ws_probe(host: str, port: int, bug_host: str) -> Optional[int]:
    """
    HTTP WebSocket Upgrade probe — الاختبار الحقيقي لـ Downlink.
    يقبل: 101 WS / 200 / 400+CF / 403+CF
    يرفض: 530 (CF error=SNI خاطئ) / 5xx / فارغ
    """
    if not deadline_ok(): return None
    ctx = _ssl_ctx()
    for path in ("/ws", "/", "/v2ray"):
        try:
            t0   = time.perf_counter()
            ...
            if status == 101:
                ai_bug_update(bug_host, True); return elapsed
            if status in (200, 301, 302):
                ai_bug_update(bug_host, True); return elapsed
            if status in (400, 403, 404, 426):
                rl = resp[:512].lower()
                cf = (b"cloudflare" in rl or b"cf-ray" in rl
                      or b"server: cloudflare" in rl or status in (400, 426))
                if cf:
                    ai_bug_update(bug_host, True); return elapsed
            # 530 = CF error = SNI خاطئ = Downlink=0B → skip
        except Exception:
            pass
...
def build_message(cfg: V2Config) -> str:
    ...
    probe_lbl = f"{cfg.probe_ms}ms" if cfg.probe_ms else "—"
    nc = len(cfg.compatible_hosts); nt = len(ALL_BUG_HOSTS)
    compat_str = "\n".join(f"   • <code>{h}</code>" for h in cfg.compatible_hosts) or "—"
    return (
        f"🤖 <b>Ashaq Team — AI Hunter v8</b> 🤖\n"
        f"━━━━━━━━━━━━━━━━━━━━━━━━━━\n"
        f"🌍 <b>Country:</b>   {cfg.country_code}  {cfg.country}\n"
        f"🔹 <b>Protocol:</b>  {cfg.proto} | {type_lbl}\n"
        f"🔒 <b>SSL/TLS:</b>   {ssl_lbl}\n"
        f"⚡ <b>TCP Ping:</b>  {cfg.ping_ms}ms — {_ping_lbl(cfg.ping_ms)}\n"
        f"🌐 <b>HTTP Probe:</b> {probe_lbl}\n"
        f"🏢 <b>ISP:</b>       {cfg.isp or 'Unknown'}\n"
        f"━━━━━━━━━━━━━━━━━━━━━━━━━━\n"
        f"📡 <b>Networks:</b>  {_operators(cfg)}\n"
        f"⭐ <b>Rating:</b>    {_rating(cfg)}\n"
        f"🗂 <b>Bug Hosts ({nc}/{nt}) — اختر ما يناسب شبكتك:</b>\n"
        f"{compat_str}\n"
        f"━━━━━━━━━━━━━━━━━━━━━━━━━━\n"
        f"ℹ️ <b>الكونفيج بدون هوست</b> — أضف البوغ هوست بنفسك\n"
        f"━━━━━━━━━━━━━━━━━━━━━━━━━━\n"
        f"🤖 <i>{cfg.ai_diagnosis}</i>\n"
        f"━━━━━━━━━━━━━━━━━━━━━━━━━━\n"
