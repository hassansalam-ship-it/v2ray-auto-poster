...
# نسخة الكاش — نرفض الكاش القديم إذا كان من نسخة مختلفة
_CACHE_VERSION = "v8-strict-101"   # غيّرها عند كل إصلاح جوهري

def cache_load() -> list[dict]:
    try:
        data  = json.loads(open(_CACHE_FILE, encoding="utf-8").read())
        # رفض الكاش القديم من نسخ سابقة
        if isinstance(data, list) and data:
            if data[0].get("cache_ver") != _CACHE_VERSION:
                log.info(f"♻️  Cache version mismatch — discarding old cache")
                return []
        fresh = [c for c in data if time.time() - c.get("ts", 0) < _CACHE_TTL]
        log.info(f"♻️  Cache: {len(fresh)}/{len(data)} (< {_CACHE_TTL//3600}h, ver={_CACHE_VERSION})")
        return fresh
    except Exception:
        return []
...
def cache_save(cfgs: list) -> None:
    try:
        ...
        for c in cfgs[:200]:
            data.append({
                "cache_ver": _CACHE_VERSION,        # للتحقق عند التحميل
...
# موفرون معروفون بحقن SNI ثابت لا يمكن تغييره (Provider-locked)
_PROVIDER_LOCKED = (
    "zain.com", "oodi.com", "mobily.com", "stc.", "ooredoo.",
    "etisalat.", "du.ae", "vodafone.", "tedata.", "te.eg",
    "batelco.", "omantel.", "nawras.",
)

def is_embedded(raw: str) -> bool:
    """
    Smart filter — يرفض فقط SNI المقفول من الموفر.
    
    المبدأ الصحيح:
    - الـ probe يحقن Bug Host بدلاً من SNI الأصلي
    - لذا SNI الأصلي لا يهم = أي domain يمر
    - نرفض فقط: موقع الموفر المباشر (oodi.com, zain.com)
      لأن هذه تعني أن الكونفيج مقفول لشبكة محددة
    """
    sni = extract_sni(raw).lower().strip()
    if not sni: return False  # فارغ = نظيف دائماً
    # موفر مقفول = ارفض
...
def ws_probe(host: str, port: int, bug_host: str, path: str = "/ws") -> Optional[int]:
    """
    SMART PROBE — متوازن بين الصرامة والعملية

    المنطق المُدروس:
    ✅ 101 = WebSocket upgrade نجح = proxy chain نشط 100%
    ✅ 400 + CF-Ray + v2ray-path = CF يستقبل لكن يحتاج v2ray client
         (v2ray يتعامل مع 400 بشكل خاص — هذا سلوك طبيعي)
    ✅ 200 + WS headers + non-CF = VPS WebSocket server
    ❌ 400 بدون CF-Ray = server آخر يحجب
    ❌ 530/5xx/timeout = ميت
    ❌ 400 + CF-Ray على path عام (/) = CF موجود لكن UUID قد يكون غير نشط
    """
    if not deadline_ok(): return None
    ctx = _ssl_ctx()
    # V2RAY_PATHS = paths التي تدل على v2ray worker حقيقي
    V2RAY_PATHS = {"/linkvws", "/vws", "/ws", "/vmess", "/vless", "/ray", "/xray"}
    ...
    for try_path in probe_paths:
        try:
            ...
            # User-Agent مطابق لـ v2ray client الحقيقي
            req  = (
            ...
            is_v2ray_path = try_path in V2RAY_PATHS or bool(
                re.match(r"^/(ws|vws|linkvws|ray|xray|vmess|vless|proxy)", try_path, re.I))
            ...
            # ✅ 200 + WS headers = WebSocket server حقيقي (VPS غالباً)
            if status == 200 and has_ws:
                log.debug(f"  ✅200+WS {host}←{bug_host} {elapsed}ms")
                ai_bug_update(bug_host, True)
                return elapsed

            # ✅ 400 + CF-Ray + v2ray path = CF Worker موجود وينتظر v2ray client
            # v2ray client يتعامل مع هذا الرد بشكل صحيح
            if status == 400 and is_cf and is_v2ray_path:
                log.debug(f"  ✅400+CF+v2path {host}←{bug_host}{try_path} {elapsed}ms")
                ai_bug_update(bug_host, True)
                return elapsed

            # ❌ كل شيء آخر
            log.debug(f"  ❌{status} cf={is_cf} path={try_path} {host}←{bug_host}")
...
def check_raw(raw: str) -> Optional[V2Config]:
    ...
    if CUSTOM_SNI:
        ...
        # probe بطيء = اتصال ضعيء = تجربة سيئة
        if probe_ms > 1500: return None
...
def main() -> None:
    ...
    # ── 1. Load + RE-VERIFY cache ────────────────────────────────────────────
    # الكاش لا يُنشر مباشرة — يُعاد فحصه أولاً (هذا يحل مشكلة Downlink=0B من الكاش)
    cached_raw  = cache_load()
    cached_objs = cache_to_configs(cached_raw)
    if cached_objs:
        log.info(f"♻️  Re-verifying {len(cached_objs)} cached configs ...")
        verified_cached: list[V2Config] = []
        for c in cached_objs[:20]:   # أقصى 20 كونفيج محفوظ نعيد فحصهم
            if not deadline_ok(): break
            # فحص سريع: TCP ping فقط + probe واحد
            p = tcp_ping(c.host, c.port)
            if p is None or p > MAX_PING_MS:
                log.debug(f"  ♻️❌ {c.host} TCP failed — removed from cache")
                continue
            # فحص probe بالـ Bug Host الأفضل المحفوظ
            best_bh = c.best_bug_host or (c.compatible_hosts[0] if c.compatible_hosts else "")
            if best_bh:
                path = extract_path(c.raw) if c.raw else "/ws"
                ms   = ws_probe(c.host, c.port, best_bh, path=path)
                if ms is None:
                    log.debug(f"  ♻️❌ {c.host}←{best_bh} probe failed — removed from cache")
                    continue
                c.probe_ms = ms   # تحديث الـ probe_ms
            verified_cached.append(c)
            log.debug(f"  ♻️✅ {c.host} re-verified (ping={p}ms)")
        log.info(f"♻️  {len(verified_cached)}/{len(cached_objs)} cached configs re-verified")
