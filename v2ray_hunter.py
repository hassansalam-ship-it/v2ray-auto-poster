...
def multi_probe(host: str, port: int) -> tuple[list[str], str, int]:
    ...
    # ── المرحلة 1: فحص متوازي لكل Bug Host ─────────────────────────
    ...
    # ── المرحلة 2: Re-verify — تحقق ثانٍ مستقل لكل ناجح ───────────
    # هذا يحذف الـ false positives ويضمن الدقة 100%
    verified: dict[str,int] = {}
    with ThreadPoolExecutor(max_workers=min(len(timings), 6)) as ex:
        rev_futures = {
            ex.submit(http_ws_probe, host, port, bh): bh
            for bh in timings
        }
        for fut in as_completed(rev_futures):
            bh = rev_futures[fut]
            try:
                ms2 = fut.result()
                if ms2 is not None:
                    # نأخذ المتوسط بين المحاولتين
                    avg = (timings[bh] + ms2) // 2
                    verified[bh] = avg
                    compat.append(bh)
                else:
                    log.debug(f"  ⚠️ Re-verify failed: {host}←{bh} (false positive removed)")
                    ai_update(bh, False)  # تصحيح الذاكرة
            except Exception:
                pass
