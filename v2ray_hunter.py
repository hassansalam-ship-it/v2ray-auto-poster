...
def _inject_vless_sni(raw: str, sni: str) -> str:
    """
    Clears existing SNI/host values, sets custom SNI, forces:
      - sni=<sni>  host=<sni>  (empty if no custom SNI)
      - path=%2Fws (encoded /ws for mobile app compatibility)
      - type=ws
      - allowInsecure=1
    """
    result = raw

    # ── 1. Clear / replace sni and host ───────────────────────────
...
def _inject_vmess_sni(raw: str, sni: str) -> str:
    """
    Decodes VMess JSON, clears SNI/host fields, sets:
      - sni=""  host=""  (empty — no built-in SNI leaks)
      - net=ws  path=/ws
      - tls=tls
      - allowInsecure=true (mapped to skip-cert-verify)
    Then re-encodes to base64.
    """
    try:
        b64 = raw[len("vmess://"):]
        # Robust b64 decode with multiple padding attempts
        for pad in ("", "=", "==", "==="):
            try:
        ...
        # ── Clear / replace sni and host ──────────────────────────
        obj["sni"]  = ""    # force empty so app uses our custom SNI
        obj["host"] = ""    # force empty CDN host header
        ...
        # ── Allow insecure (self-signed certs) ────────────────────
        obj["allowInsecure"] = True          # v2rayNG / Hiddify field
        obj["skip-cert-verify"] = True       # clash/mihomo field

        # ── Preserve other fields untouched ───────────────────────
...
def build_message(cfg: V2Config) -> str:
    ...
    sni_line   = f"🔑 <b>SNI:</b> <code>{cfg.active_sni}</code>  [{sni_src}]" if cfg.active_sni else "🔑 <b>SNI:</b> —"
    return (
...
def main() -> None:
    ...
    for i, c in enumerate(live[:10], 1):
        ...
        log.info(f"  {i:>2}. {flags} [{c.ping_ms:>4}ms] {c.country_code} {c.proto} sni={c.active_sni or '—'} {c.host[:35]}")
